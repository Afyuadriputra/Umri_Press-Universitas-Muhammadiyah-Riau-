<x-surat-layout>
    ...
    <div class="space-y-6">
        <div>
            <h1 class="text-2xl font-semibold text-neutral-900 dark:text-neutral-100">Edit Surat Masuk</h1>
            <p class="text-sm text-neutral-500 dark:text-neutral-400">Perbarui data surat masuk.</p>
        </div>

        <form method="POST" action="{{ route('dashboard-surat.incoming.update', $letter) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <x-text-input type="date" name="received_at" value="{{ old('received_at', optional($letter->received_at)->format('Y-m-d')) }}" class="w-full" required />
                ...
                <div>
                    ...
                    <x-text-input type="date" name="letter_date" value="{{ old('letter_date', optional($letter->letter_date)->format('Y-m-d')) }}" class="w-full" />
                ...
                <div>
                    ...
                    <x-text-input type="text" name="sender" value="{{ old('sender', $letter->sender) }}" class="w-full" required />
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="summary" rows="3"
                        class="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-800 focus:border-cgreen-500 focus:ring-cgreen-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100">{{ old('summary', $letter->summary) }}</textarea>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <x-select name="assigned_user_id" class="w-full">
                        ...
                        @foreach ($staff as $user)
                            <option value="{{ $user->id }}" @selected(old('assigned_user_id', $letter->assigned_user_id) == $user->id)>{{ $user->name }}</option>
                        @endforeach
                    </x-select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="disposition_note" rows="3"
                        class="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-800 focus:border-cgreen-500 focus:ring-cgreen-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100">{{ old('disposition_note', $letter->disposition_note) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="internal_notes" rows="3"
                        class="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-800 focus:border-cgreen-500 focus:ring-cgreen-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100">{{ old('internal_notes', $letter->internal_notes) }}</textarea>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="md:col-span-2 text-xs text-neutral-500 dark:text-neutral-400">
                    @if ($letter->scan_path)
                        <div>
                            Scan saat ini: <a href="{{ asset($letter->scan_path) }}" target="_blank" class="text-cgreen-600 underline">Lihat</a>
                        </div>
                    @endif
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('dashboard-surat.incoming.show', $letter) }}" class="inline-flex items-center rounded-lg border border-neutral-200 px-4 py-2 text-sm font-semibold text-neutral-700 hover:bg-neutral-50 dark:border-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-800">
                    Batal
                </a>
