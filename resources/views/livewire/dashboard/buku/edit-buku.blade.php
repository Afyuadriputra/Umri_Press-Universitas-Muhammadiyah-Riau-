        ...
        <form wire:submit.prevent="save" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="space-y-2">
                    ...
                    @enderror
                    @php
                        $hargaAngka = is_numeric($harga) ? (int) $harga : null;
                        $diskonHard = is_numeric($diskon_hard) ? (int) $diskon_hard : 0;
                        $hargaSetelahDiskon =
                            $hargaAngka !== null
                                ? max(0, $hargaAngka - ($hargaAngka * $diskonHard) / 100)
                                : null;
                    @endphp
                    @if (!is_null($hargaAngka))
                        <div class="text-xs text-neutral-500 dark:text-neutral-400">
                            Setelah diskon: <span class="font-semibold text-cgreen-600 dark:text-cgreen-400">Rp {{ number_format($hargaSetelahDiskon, 0, ',', '.') }}</span>
                        </div>
                    @endif
                </div>
                ...
                <div class="space-y-2">
                    ...
                    @php
                        ...
                        $hargaSoftDiskon = $hargaSoft !== null
                            ? max(0, $hargaSoft - ($hargaSoft * $diskonSoft) / 100)
                            : null;
                    @endphp
            ...
            <div class="mt-6 space-y-3">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="mt-2 space-y-1">
                        ...
                        @if ($oldEbook)
                            <p class="text-sm text-neutral-600 dark:text-neutral-400">File saat ini:
                                <a href="{{ asset($oldEbook) }}" target="_blank" class="text-cgreen-600 underline">Lihat PDF</a>
                            </p>
                        @endif
                        @error('ebook')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                        <p class="text-xs text-neutral-500">Wajib jika opsi softfile dicentang dan belum ada file.</p>
                    </div>
                    <div class="mt-4 space-y-1">
                        ...
                        @if ($oldPreview)
                            <p class="text-sm text-neutral-600 dark:text-neutral-400">Preview saat ini:
                                <a href="{{ asset($oldPreview) }}" target="_blank" class="text-cgreen-600 underline">Lihat PDF</a>
                            </p>
                        @endif
                        @error('preview_pdf')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                        <p class="text-xs text-neutral-500">Upload beberapa halaman untuk preview (maks 10MB).</p>
                        <div class="mt-2">
